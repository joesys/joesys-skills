...
Read `../shared/skill-context.md` for the full protocol (resolve `../shared/...` against the collection root (one level above this SKILL.md) - never the project's working directory). This skill is **full interview** category.

1. Check for `.codex/skill-context/preferences.md` -- if missing, invoke `$preferences` before proceeding.
2. Check for `.codex/skill-context/handbook.md` -- if found, load previous interview answers and context.
...
**MUST dispatch all 6 agents in a single message using the Codex agent workflow.** All agents use `model: "opus"`. Sequential dispatch is a defect.
...
1. If `.codex/skill-context/handbook.md` exists, check which questions have already been answered in previous runs. **Skip those** -- only ask about new gaps.
...
4. Save all answers to `.codex/skill-context/handbook.md` with timestamps.
...
All interview answers are persisted to `.codex/skill-context/handbook.md`. On subsequent runs:
...
`.codex/skill-context/handbook.md` is a bulleted markdown file: a `# Handbook Preferences` heading with a `Last updated: <DATE>` line, then one `##` section per question category (Design Rationale, Extension Points, Danger Zones, Tribal Knowledge, Style & Conventions, Corrections), each holding `- <question>: <answer>` bullets.

---

## Phase 3 -- Parallel Chapter Writers

**MUST dispatch all chapter writer agents in a single message using the Codex agent workflow.** All agents use `model: "opus"`. Sequential dispatch is a defect.
...
| `.codex/skill-context/handbook.md` | Persisted interview context for future runs |
...
5. **Interview context is persistent** -- Answers saved to `.codex/skill-context/handbook.md` carry forward to future runs, preventing repeated questions.
...
7. **No code modification** -- The skill is read-only with respect to source code. No files outside `docs/handbook/` and `.codex/skill-context/handbook.md` are written.
