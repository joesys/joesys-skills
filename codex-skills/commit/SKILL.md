...
**Preference gate:** If the user's commit preferences (`.codex/skill-context/commit.md`) set `auto-devlog scrap: off`, skip this entire section. Some users prefer not to auto-generate devlog scraps - respect that choice.
...
2. **There is something genuinely worth capturing from this session.** Type-significance is necessary but not sufficient - most `feat`/`fix`/`refactor` commits are routine and produce nothing worth re-reading later. Honestly judge whether the work behind this commit yielded an insight that the user, six months from now, would want to revisit. Apply the capture bar defined in the devlog skill (the "What's Worth Capturing" section of the sibling file `../devlog/SKILL.md`, at the plugin's `../devlog/`); **MUST bias toward NO when uncertain.** Too many scraps is noisy and dilutes the value of the ones that actually matter.
