...
1. List `~/.claude/sessions/*.json`. Each pointer file is written by Claude Code and records the Claude Code process `pid`, its `cwd`, the `sessionId`, and a start timestamp - do NOT use the shell command tool's `$$`, which is a transient subprocess PID that never matches these files.
