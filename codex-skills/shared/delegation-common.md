...
5. **MUST use 600000ms timeout** on every shell dispatch. CLI runs are bursty; shorter timeouts produce false failures.
...
Use `mktemp` to create temp files portably (Linux, macOS, Windows Git shell):
...
**Why `2>/dev/null`?** CLI tools emit progress indicators, ANSI escape codes, and status messages on stderr. These pollute captured output when running programmatically. **Exception:** Codex prints its `session id:` banner on stderr — when resume matters, capture stderr to a temp file instead (see `./model-defaults.md` § Codex Resume).
...
If the user invokes a resume command with no prompt (e.g., `$codex resume`), use `ask the user directly` to ask what they want to follow up on **before** dispatching. Never dispatch an empty prompt.
...
- **Research disagreements** using web search or project documentation before accepting claims.
...
When the host model evaluates output from the same family (Claude evaluating Claude), shared blind spots compound. **MUST flag** when independent verification (web search, docs) would add value over two instances of the same model agreeing.
...
These delegation skills expose resume interfaces documented in `./skill-interfaces.md`. **Before modifying** the resume syntax or behavior, **MUST update** the interface contract and every caller listed there.
