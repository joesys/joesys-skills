...
Per-language static analysis tools for JavaScript projects. Read this file alongside `./tooling/general.md`. All tools follow the safety rules, detection flow, and execution protocol defined in `./tooling-registry.md`.
...
| **Recommendation** | Best-in-class for JavaScript security scanning. Covers XSS, injection, insecure deserialization, prototype pollution, and more via the `auto` ruleset. See also `./tooling/general.md` for the multi-language Semgrep profile. |
