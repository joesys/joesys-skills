...
Present the narrative arc, key metrics, and proposed topic list. Ask using `ask the user directly`:
...
2. **Ask the human to react** using `ask the user directly`:
...
Present improvement proposals **one by one** using `ask the user directly`: Approve / Reject / Modify. Update status in the file. **MUST NOT apply changes without explicit approval.**
...
| Skipping human reaction | Human perspective is the most valuable input | Always use `ask the user directly` |
