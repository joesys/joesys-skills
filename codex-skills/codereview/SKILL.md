...
**MUST fire all cluster dispatches in a single parallel batch.** With N clusters, that's N x 9 tool calls in one response. No user gate between scoping and dispatch - the scoping pass returns, dispatch fires automatically.
...
**MUST dispatch 7 subagents simultaneously** via the Codex agent workflow - all 7 in a single response (7 parallel Codex agent workflow calls). Each subagent is a domain expert that analyzes the code against one principle set. Sequential dispatch is a defect.
...
- Dispatch parallel fix agents for independent groups via the Codex agent workflow
