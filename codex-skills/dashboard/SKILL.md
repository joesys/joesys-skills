...
1. **Always** run the git collector, writing the canonical artifact:
   ```bash
   python ../dashboard/helpers/collect_git.py --repo <repo> --out docs/dashboard/dashboard.json
   ```
...
2. **Unless `--no-host`**, run the host collector to a temp file:
   ```bash
   python ../dashboard/helpers/collect_host.py --repo <repo> --out /tmp/host.json
   ```
...
3. **Always** run the audit reader to a temp file (it is cheap and read-only):
   ```bash
   python ../dashboard/helpers/collect_audit.py --repo <repo> --out /tmp/audit.json
   ```
   When no report exists it writes `{"available": false}`.

> On Windows, substitute a writable temp path for `/tmp/` (in the shell command tool's Git shell, `$TEMP` or an absolute path like `C:/Users/<you>/AppData/Local/Temp`). The collectors only need a path they can write the small JSON to.
...
```bash
python ../dashboard/helpers/render_dashboard.py --metrics docs/dashboard/dashboard.json --out docs/dashboard/dashboard.html
```
