...
1. Read `.codex/skill-context/ss.md` in the current project.
...
3. Validate the folder exists using the shell command tool (`ls "<path>"`). If invalid, tell the user and re-prompt.

4. Write the path to `.codex/skill-context/ss.md`:
   ```markdown
   ## Screenshot Skill Config
   - **Screenshot folder:** <path>
   ```

5. Save the path to the project's auto-memory as a reference memory (e.g., `reference_screenshot_folder.md`). Auto-memory is per-project, so this pre-populates future runs *in this project*, not other projects - the reliable per-project store is `.codex/skill-context/ss.md` from step 4. If a memory file already exists, update it.

### 1.2 Subsequent Runs

Read `.codex/skill-context/ss.md` and verify the folder exists. If gone, re-enter the bootstrap flow at step 2 (global-memory check).
...
Use the shell command tool to list matching files sorted by modification time (newest first). Adapt to the platform - for example:
...
Only when N > 1: Use the shell command tool to get modification timestamps. If any file is **more than 1 hour older** than the next-newest file in the batch, flag it:
